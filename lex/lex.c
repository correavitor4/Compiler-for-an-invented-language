...
Token *tokens = NULL; // vetor de tokens
...
short read_files()
{
    ...
    struct dirent *entry;
    char filepath[1024]; // Buffer para armazenar o caminho completo do arquivo
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        // Cria FileReader
        FileReader *fr = filereader_create(filepath);
        if (!fr)
        {
        ...
        {
            // Aqui você chama a função que processa a linha e gera tokens
            // ex: lex_line(line);
        }
...
int next_token(const char *line, int *current_position)
{
    ...
    int char_accumulator_size = get_initial_lex_token_accumulator_size();
    ...
    {
        char_accumulator[*char_accumulator] = line[*current_position];
...
{
    if (char_accumulator_size == 0) return 0;
